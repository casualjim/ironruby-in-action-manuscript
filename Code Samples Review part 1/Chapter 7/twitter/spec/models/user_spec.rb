...
fixtures :users, :follower_users, :statuses
 ...
 if REST_AUTH_SITE_KEY.blank?
  ...
  describe "relations" do
    ...
    it "should include statuses" do
      user = users(:quentin)
      user.should_not be_nil
      user.statuses.should_not be_nil
      user.statuses.should_not be_empty
      user.statuses.count.should == 3
    end
    
    it "should include followers" do
      user = users(:quentin)
      user.should_not be_nil
      user.followers.should_not be_empty
      user.followers.count.should == 2      
    end
    
    it "should include following" do
      user = users(:aaron)
      user.should_not be_nil
      user.following.should_not be_empty
      user.following.count.should == 1
      user.following.first.id.should == 1   
    end
  end
  
  context "should find all the statuses for the user and his friends" do
    ...
    specify "when the user has (a) friend(s)" do
      ...
      user.friends_timeline.should_not be_empty
      user.friends_timeline.length.should == 5
    end
  ...
  context "finders" do
    ...
    specify "should find the statuses for the user" do
      user = users(:quentin)
      user.should_not be_nil
      user.timeline.should_not be_empty
      user.timeline.length.should == 3
    end

    specify "should find a user by login or id when an id is specified" do
      usr = User.find_by_id_or_login 1
      usr.should_not be_nil
      usr.id.should == 1
      usr.login.should == "quentin"
    end

    specify "should find a user by login or id when a login is specified" do
      usr = User.find_by_id_or_login "quentin"
      usr.should_not be_nil
      usr.id.should == 1
      usr.login.should == "quentin"
    end
